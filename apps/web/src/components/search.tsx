...
function SearchButton({ onOpen }: { onOpen: () => void }) {
	return (
		<button
			type="button"
			className="inline-flex h-9 w-fit items-center gap-3 rounded-md border border-input bg-background px-3 py-2 text-foreground text-sm shadow-xs outline-none transition-[color,box-shadow] placeholder:text-muted-foreground/70 focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50"
			onClick={onOpen}
		>
			<SearchIcon
			...
			<kbd className="inline-flex h-5 max-h-full items-center rounded border bg-background px-1 font-medium text-[0.625rem] text-muted-foreground/70">
				{KEYBOARD_SHORTCUT}
			</kbd>
		</button>
	);
